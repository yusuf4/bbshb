                ...
                <div class="rounded-[10px] bg-white p-4 !pt-10 sm:p-6">
                    <div ref="pdfContent">
                        <div class="pb-2 border-b">
                            <span class="block text-lg font-medium">{{cardB.name}}</span>
                        </div>
                        ...
                        <div class="flex  items-start mt-8">
                            ...
                            <div class="flex flex-col  mb-2">
                                <span class="text-sm text-gray-700 pb-2">{{formated(cardB.created_at)}}</span>
                                <span class="text-sm text-gray-700 pb-2">{{cardB.nomer_b.id}}</span>
                                <div class="flex">
                                        <span
                                            v-for="fileS in cardB.fileshartnoma_b"
                                            :key="fileS.id"
                                            class="text-sm text-blue-600 pb-2 pr-2"
                                        ><a
                                            :href="'/uploads/shartnoma/'+fileS.name" target="_blank">{{fileS.name}},</a></span>
                                </div>
                                <span class="text-sm text-gray-700 pb-2">{{cardB.namud_b.name}}</span>
                                <div
                                    v-if="cardB.countries_b.length>0"
                                    class="flex">
                                    <span
                                        v-for="mintaqa in cardB.countries_b"
                                        :key="mintaqa.id"
                                ...
                                <span class="text-sm text-gray-700 pb-1.5">{{formated(cardB.sanai_etibor)}}</span>
                                <span class="text-sm text-gray-700 pb-2">{{cardB.tartibi_etibor_b.name}}</span>
                                <div class="flex">
                                    <span
                                        v-for="file in cardB.file_bisyor"
                                        :key="file.id"
                                        class="text-sm text-blue-600 pb-2.5 pr-2"
                                    ><a :href="'/uploads/files/'+file.name" target="_blank">{{file.name}},</a></span>
                                </div>
                                <span class="text-sm text-gray-700 pb-2" v-if="cardB.file_bisyor<=0">----</span>
                                <span class="text-sm text-gray-700 pb-2">{{cardB.muhlati_etibor_b.name}}</span>
                                <span class="text-sm text-gray-700 pb-1.5" v-if="cardB.qati_etibor!=null">{{formated(cardB.qati_etibor)}}</span>
                                <span class="text-sm text-gray-700 pb-1.5" v-else>-----</span>
                                <span class="text-sm text-gray-700 pb-2">{{cardB.maqomot}}</span>
                                <div
                                    v-if="cardB.ezoh_b.length>0"
                                    class="flex">
                                    <span
                                        v-for="ezoh in cardB.ezoh_b"
                                        :key="ezoh.id"
                                        class="text-sm text-gray-700 pb-2 pr-2"
                                    >{{ezoh.name}},</span>
                                </div>
